...
# 1. 读取时间数据
# ===============================
time_path = 'E://CSFSL/STNN_NGSIM/CT.csv'
time_df = pd.read_csv(time_path, header=None, sep=r'\s+')
LIST_Time = time_df.values.astype(np.float32)   # shape: [T, X]
# 如果后续代码用的是 torch
LIST_Time = torch.from_numpy(LIST_Time)
print('LIST_Time shape:', LIST_Time.shape)

# ===============================
# 2. 读取关系矩阵
# ===============================
relation_path = 'E://CSFSL/STNN_NGSIM/CT_relation.csv'
relation_df = pd.read_csv(relation_path, header=None, sep=r'\s+')
LIST_Relation = relation_df.values.astype(np.float32)  # shape: [X, X]
LIST_Relation = torch.from_numpy(LIST_Relation)
print('LIST_Relation shape:', LIST_Relation.shape)


# data_0 = CT_data.LIST_Time
# relation_0 = CT_data.LIST_Relation
data_0 = LIST_Time
relation_0 = LIST_Relation
# data_0 = FAQ_data.LIST_Time
# relation_0 = FAQ_data.LIST_Relation
# data_0 = NGSIM_data222.LIST_Num[1]
# relation_0 = NGSIM_data222.LIST_Relation[1]
# np.savetxt('CT.csv', data_0, delimiter=' ')
# np.savetxt('CT_relation.csv', relation_0, delimiter=' ')
...
# -- xp
p.add('--outputdir', type=str, help='path to save xp', default='output1.6')  # 保存实验结果的径
...
p.add('--khop', type=int, help='spatial depedencies order', default=1)  # 空间 依赖关系阶数
...
p.add('--beta1', type=float, default=.9, help='adam beta1')  # Adam优化器的beta1参数
p.add('--beta2', type=float, default=.999, help='adam beta2')  # Adam优化器的beta2参数
p.add('--eps', type=float, default=1e-8, help='adam eps')  # Adam优化器的epsilon参数
p.add('--wd', type=float, help='weight decay', default=1e-4)  # 权重衰减系数
p.add('--wd_z', type=float, help='weight decay on latent factors', default=1e-4)  # 潜在因素上的权重衰减系数
...
p.add('--nepoch', type=int, default=700, help='number of epochs to train for')  # 训练的总epoch数
# -- gpu
p.add('--device', type=int, default=-1, help='-1: cpu; > -1: cuda device id')
# -- seed
p.add('--manualSeed', type=int, default=9322, help='manual seed')
...
setup, (train_data, test_data), relations = dataset_factory(data_0, relation_0, 10, opt.khop)
...
# 加载训练好的模型
model_0 = torch.load('E://CSFSL/STNN_NGSIM/output1.5/stnn/model.pt')
...
# # 初始化模型后用训练好的参数替换掉原始参数
# tensor_f = model.factors.clone()
# train_tensor = torch.nn.Parameter(model_0['factors'])
# tensor_f = train_tensor[0:100, 0:30, :]
#
# tensor_f = train_tensor[:60, :9, :]
#
# tensor_f[:100, :30, :] = train_tensor
# tensor_f[:100, 30:60, :] = train_tensor
# tensor_f[:100, 60:90, :] = train_tensor
# tensor_f[:100, 90:120, :] = train_tensor
# tensor_f[:100, 120:150, :] = train_tensor
# tensor_f[:100, 150:180, :] = train_tensor
# tensor_f[:100, 180:210, :] = train_tensor
# tensor_f[:100, 210:240, :] = train_tensor
# tensor_f[:100, 240:270, :] = train_tensor
# tensor_f[:100, 270:300, :] = train_tensor
# tensor_f[:100, 300:330, :] = train_tensor
# tensor_f[:100, 330:360, :] = train_tensor
# tensor_f[:100, 360:390, :] = train_tensor
# tensor_f[:100, 390:420, :] = train_tensor

# 当前模型的 factors（target 结构）
tensor_f = model.factors.data.clone()

# 预训练 factors（source）
src_factors = model_0['factors']  # shape: (T_src, 9, nz)

# 你可以按需要裁剪时间维（示例）
T = min(tensor_f.size(0), src_factors.size(0))
tensor_f = tensor_f[:T]

# ---------- 核心迁移逻辑 ----------
# 用 source 的 9 个区域，去填充 target 的 30 个区域
# 例如：简单重复填充（你原来就是这个思路）

for i in range(0, tensor_f.size(1), src_factors.size(1)):
    end = min(i + src_factors.size(1), tensor_f.size(1))
    tensor_f[:, i:end, :] = src_factors[:T, :end - i, :]


# model.factors = torch.nn.Parameter(tensor_f)
model.factors.data.copy_(tensor_f)
# model.factors.data.copy_(model_0['factors'])
model.dynamic.module.weight.data.copy_(model_0['dynamic.module.weight'])
model.dynamic.module.bias.data.copy_(model_0['dynamic.module.bias'])
model.decoder.weight.data.copy_(model_0['decoder.weight'])
...
for e in pb:
    ...
    if nex_dyn != 0:
        ...
        logs_train['mse_dyn'] = 10000
        logs_train['loss_dyn'] = 10000
    ...
    with torch.no_grad():
        ...
        # # 将Tensor转化为二维数组
        # numpy_arr_pre = x_pred.numpy().reshape(-1, 9)
        # numpy_arr_test = test_data.numpy().reshape(-1, 9)
        # df_pre = pd.DataFrame(numpy_arr_pre)
        # df_test = pd.DataFrame(numpy_arr_test)
        # # index表示是否加索引
        # df_pre.to_csv('pred.csv', index=False)
...
# 使用加载的字体
plt.title('数据集crowd_temperature下的Loss曲线图', fontproperties=font, fontsize=14)
plt.xlabel('周期', fontproperties=font, fontsize=14)
plt.ylabel('loss')
# plt.xlim((0, 6000))
# plt.ylim((0, 30000))
plt.legend()
...
# 使用加载的字体
plt.title('数据集crowd_temperature下的RMSE曲线图', fontproperties=font, fontsize=14)
plt.xlabel('周期', fontproperties=font, fontsize=14)
plt.ylabel('RMSE')
# plt.xlim((0, 6000))
# plt.ylim((0, 10000))
