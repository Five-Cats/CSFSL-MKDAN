...
class MSDAN(nn.Module):
    ...
    def forward(self, x, domain_idx=None, is_target=False):
        ...
        feat_mean_save = st_feat.mean(dim=1)  # (B, N, D)
        ...
        if (domain_idx is not None) or is_target:
            ...
            # 返回
            return pred, domain_loss, mmd_loss, feat_mean_save

        # 如果不进行域自适应（如测试），保留原行为
        return pred, torch.tensor(0.0, device=self.device), torch.tensor(0.0, device=self.device), feat_mean_save
    ...
    for epoch in range(epochs):
        ...
        for domain_idx, (city, loader) in enumerate(data_loader.source_train_loaders.items()):
            for x_src, y_src in loader:
                ...
                pred_src, domain_loss_src, _, _ = model(
                    x_src,
                ...
                if x_tgt is not None:
                    pred_tgt, domain_loss_tgt, _, _ = model(
                        x_tgt,
                        domain_idx=None,
                        is_target=True,
                    )
...
def finetune_on_target(model, data_loader, epochs, device="cuda", mmd_weight=0.1, target_city=None):
    """
    Stage 2: Fine-grained Distribution Alignment (MMD) on target domain.

    1. 每个 epoch 清空 source_feats（防止污染）
    2. 每个 batch 用 source 更新 source_feats（MMD有参照）
    3. source forward 不参与梯度（no_grad）
    4. target 再进行 MMD 对齐
    """
    ...
    source_iters = {
        i: iter(loader)
        for i, loader in enumerate(data_loader.source_train_loaders.values())
    }
    ...
    for epoch in range(epochs):
        # 每个epoch清空source_feats
        for i in range(model.num_sources):
            model.source_feats[i] = []
        ...
        for x_tgt, y_tgt in data_loader.target_train_loader:
            ...
            # 用source 数据更新source_feats
            for domain_idx, loader in enumerate(data_loader.source_train_loaders.values()):
                try:
                    x_src, _ = next(source_iters[domain_idx])
                except StopIteration:
                    source_iters[domain_idx] = iter(loader)
                    x_src, _ = next(source_iters[domain_idx])
                ...
                # 只forward，不反向传播
                with torch.no_grad():
                    model(x_src, domain_idx=domain_idx, is_target=False)

            # target forward（计算MMD）
            pred, _, mmd_loss, _ = model(x_tgt, domain_idx=None, is_target=True)
...
def evaluate(model, loader, device="cuda", target=False, max_val=None, min_val=None):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # forward
            pred, _, _, _ = model(x, domain_idx=None, is_target=False)
...
if __name__ == "__main__":
    ...
    # 只保留两种模式
    mmd_modes = ["none", "nodesim"]
    colors = {"nodesim": "red", "none": "blue"}
    ...
    for mmd_mode in mmd_modes:
        print(f"\n--- Training with MMD mode: {mmd_mode} ---")
        ...
        # Stage 1: DANN
        # ======================
        print("=== Stage 1: Global Adversarial Alignment (DANN) ===")
        pretrain_on_sources(
            model,
            data_loader,
            epochs=150,
        ...
        # Stage 2: MMD（关键！）
        # ======================
        print("=== Stage 2: Fine-grained NodeSim-Aware MMD Alignment ===")

        # ⚠️关键修改点：
        # none → mmd_weight=0
        # nodesim → mmd_weight>0
        mmd_weight = 0.1 if mmd_mode != "none" else 0
        ...
        # 测试
        # ======================
        test_mae, test_rmse, preds = test_on_target(
            finetuned_model,
            data_loader,
            device=device,
            city=target_city
        ...
        print(f"[Final Test] Mode={mmd_mode}, RMSE={test_rmse:.4f}, MAE={test_mae:.4f}")

        # ======================
        # 保存结果
        # ======================
        experiment_results[target_city][mmd_mode] = {
        ...
        plt.plot(overfitting_gap, label=mmd_mode, color=colors[mmd_mode])
    ...
    plt.title(f"过拟合分析 - 目标城市: {target_city}", fontsize=18)
    plt.xlabel('训练轮次', fontsize=16)
    plt.ylabel('验证集RMSE - 训练集RMSE', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(f"overfitting_{target_city}.png", dpi=300)
    plt.show()
    ...
    # ======================
    print("\n====== Final Comparison ======")
    for mode, res in experiment_results[target_city].items():
        print(f"{mode}: RMSE={res['final_test_rmse']:.4f}, MAE={res['final_test_mae']:.4f}")
