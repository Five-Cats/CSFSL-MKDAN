...
class MVGAT(nn.Module):
    def __init__(self, in_feats, h_dim, num_heads=2, dropout_rate=0.3):
        super().__init__()
        ...
        self.view_proj = nn.ModuleDict({
            gtype: nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(p=dropout_rate)  # dropout防止过拟合
...
class FusionModule(nn.Module):
    def __init__(self, num_graphs, emb_dim, alpha=0.8, dropout_rate=0.3):
        super().__init__()
    ...
    def forward(self, views):
        ...
        fused = F.dropout(fused, p=self.dropout_rate, training=self.training)  # 防止过拟合
        return fused, views
...
class MSDAN(nn.Module):
    def __init__(self, in_feats, h_dim, city_graph_data_list, device, use_fusion=True, use_gcn=True, use_temporal=True,
        ...
        # 消融实验
        if self.use_fusion:
            self.mvgat = MVGAT(in_feats, h_dim, dropout_rate=dropout_rate)
            self.fusion_module = FusionModule(num_graphs=4, emb_dim=h_dim, dropout_rate=dropout_rate)
        else:
            self.mvgat = MVGAT(in_feats, h_dim, dropout_rate=dropout_rate)
...
def generate_dropout_ablation_report(experiment_results, target_city):
    print("\n" + "=" * 80)
    print(f"DOMAIN WEIGHT ABLATION REPORT - Target City: {target_city}")
    print("=" * 80)
    ...
    for w, res in experiment_results.items():
        print(
            f"{w:<10} "
            f"{res['final_test_mae']:<12.4f} "
            f"{res['final_test_rmse']:<12.4f} "
            f"{res['best_test_mae']:<12.4f} "
            f"{res['best_test_rmse']:<12.4f} "
            f"{res['overfitting_gap']:<15.4f}"
        )

    best_w = min(
        experiment_results.keys(),
        key=lambda x: experiment_results[x]['best_test_rmse']
    )

    print("\nBest Configuration:")
    print(f"  - Best domain_weight: {best_w}")
    print(f"  - Best RMSE: {experiment_results[best_w]['best_test_rmse']:.4f}")
    print(f"  - Best MAE: {experiment_results[best_w]['best_test_mae']:.4f}")

    with open(f"domain_weight_ablation_{target_city}.txt", "w") as f:
        f.write(f"Domain Weight Ablation Report - {target_city}\n")
        f.write(f"Best domain_weight: {best_w}\n")
        f.write(f"Best RMSE: {experiment_results[best_w]['best_test_rmse']:.4f}\n")
        f.write(f"Best MAE: {experiment_results[best_w]['best_test_mae']:.4f}\n")

    print(f"\nReport saved to: domain_weight_ablation_{target_city}.txt")
...
if __name__ == "__main__":
    ...
    # ================= 固定权重 =================
    FIXED_MMD_WEIGHT = 0.10
    FIXED_DOMAIN_WEIGHT = 0.10
    ...
    for target_city in all_cities:
        ...
        print(f"\nRunning dropout ablation: Target={target_city}, Sources={source_cities}")
        ...
        for i in range(len(source_cities)):
            sim = compute_city_similarity_simple(city_graph_data[i], target_graph)
        ...
        # ---------- 节点级相似度 ----------
        node_sim_matrices = [
            compute_node_level_similarity_full(city_graph_data[i], target_graph)
            for i in range(len(source_cities))
        ]
        ...
        for dropout_rate in dropout_rates:
            ...
            pretrain_on_sources(
                ...
                epochs=150,
                device=device,
                domain_weight=FIXED_DOMAIN_WEIGHT
            )
