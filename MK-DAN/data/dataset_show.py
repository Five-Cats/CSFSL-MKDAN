...
data = np.load('E:/trans/DAN/MK-DAN/data/LABike/dataset_day.npy')  # 替换为你的npy文件路径
...
plt.xticks(xticks, xticks)  # 设置刻度位置和标签

# 设置纵坐标（areas）刻度显示为1-36
yticks = np.arange(0.5, average_flow.shape[1]+0.5, 1)  # 每个小区都显示
plt.yticks(yticks, np.arange(1, 37, 1))  # 显示1-36所有标签

# 设置坐标轴标签
plt.xlabel('Days', fontsize=16)
plt.ylabel('Areas', fontsize=16)

# 将标题放在图下方，调整位置和边距
plt.title('LosAngeles Traffic Distribution',
          fontsize=18,
