...
import gc
from model.TSFormer.Transformer_layers import TransformerLayers
from model.TSFormer.mask import MaskGenerator
from model.TSFormer.patch import Patch
from model.TSFormer.positional_encoding import PositionalEncoding
from model.TSFormer.GNN import SGC
...
class TSFormer(nn.Module):
    ...
    def __init__(self, model_cfg,args, mode='Pretrain'):
        super().__init__()
        ...
        self.decoder2 = TransformerLayers(out_channel, 1)
        self.SGC = SGC(mask_size* out_channel)
        # nn.init.normal_(self.mask_token,std=.02)
        nn.init.uniform_(self.mask_token, -0.02, 0.02)

        self.output_layer = nn.Linear(out_channel, patch_size)

    def _forward_pretrain(self, input, A):
...
nn
        ...
        out_full_flat = out_full_unshuffled.reshape(B, N, LP * d)  # B, N, L/P * d
        if(self.args.use_A == 'A'):
            num_layer = 2
            A_final = torch.eye(N).to(out_full_flat.device)
            for i in range(num_layer):
                A_final = torch.einsum('ij,jk->ik', A_final, A)
        else:
            A_final = A
        out_full_SGC = self.SGC(out_full_flat, A_final)
        H_SGC = out_full_SGC.reshape(B, N, LP, d)  # B, N, L/P, d
        out_full_unshuffled = H_SGC
        # out_full_unshuffled = self.output_layer(H_SGC)
        
        shuffle_list = unmasked_token_index + masked_token_index
        out_full = out_full_unshuffled[:,:,shuffle_list,:]
        
        out_full = self.decoder2(out_full)
        
        out_full = self.output_layer(out_full)
    ...
    def _forward_backend(self, input, A, use_spatial=True):
        ...
        B, N, LP, d = H.shape
        # two layer SGC
        out_full_flat = H.reshape(B, N, LP * d)  # B, N, L/P * d
        ...
        if(use_spatial):
            if(len(A.shape)==2):
                ...
                if(self.args.use_A == 'A'):
                    num_layer = 2
                    A_final = torch.eye(N).unsqueeze(0).repeat(A.shape[0], 1, 1).to(out_full_flat.device)
                    # print(A_final.shape, A.shape)
                    for i in range(num_layer):
                        A_final = torch.einsum('bij,bjk->bik', A_final, A)
                else:
                    A_final = A
            else:
            ...
            out_full = H
        out_full = self.decoder2(out_full)
        out_full_unshuffled = self.output_layer(out_full)
        H = out_full
        
        
        return H

    def forward(self, input_data, A, use_spatial=True):
        ...
        if self.mode == 'Pretrain':
            return self._forward_pretrain(input_data, A)
        else:
            return self._forward_backend(input_data, A,use_spatial)
