...
from model.TSFormer.TSmodel_TSFormerTST import *
from model.TSFormer.GNN import *
sys.path.append('../../')
from utils import *
import datetime
...
class PatchFSL(nn.Module):
    """
    Full PatchFSL Model
    该模块用于构建时空图神经网络模型，集成图卷积、模式提取、图重建等模块
    """
    def __init__(self, data_args, model_args, task_args, PatchFSL_cfg, args, model='GWN'):
        ...
        pattern_dict = {}
        for hour in hour_list:  # 从磁盘加载预训练好的 pattern 表示（.pt文件）
...
class STRep(nn.Module):
    ...
    该模型是一个支持 Reptile/MAML 式的少样本学习框架，用于交通流预测。
    """
    ...
    def get_per_step_loss_importance_vector(self):
        ...
        用于设置每个inner loop步骤 loss权重（靠近末尾的step更重要）
        """
    ...
    "%Y%m%d-%H%M%S")
        ...
        for i in range(target_epochs):
            ...
            log_verbose(args,'in training   Unnormed MSE : {:.5f}, RMSE : {:.5f}, MAE : {:.5f}, MAPE: {:.5f}, normed MSE : {:.5f}.'.format(np.mean(total_mse), np.mean(total_rmse), np.mean(total_mae),np.mean(total_mape),np.mean(total_loss)))
        ...
        for i in range(self.task_args['maml']['pred_num']):
            ...
            log_verbose(args,'Horizon {} : Unnormed MSE : {:.5f}, RMSE : {:.5f}, MAE : {:.5f}, MAPE: {:.5f}'.format(i,np.mean(total_mse), np.mean(total_rmse), np.mean(total_mae),np.mean(total_mape)))
