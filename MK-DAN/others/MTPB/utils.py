...
def get_data_list(data_list):
    ...
    if ('nycbike' in split_data_list):
        dlist.append('nycbike')
    if ('washington' in split_data_list):
        dlist.append('washington')
    if ('chicago' in split_data_list):
        dlist.append('chicago')
    if ('labike' in split_data_list):
        dlist.append('labike')
    return dlist
...
# 将原始邻接矩阵A进行归一化处理，输出对称归一化形式的邻接矩阵
def get_normalized_adj(A):
    """
    Returns the degree normalized adjacency matrix.
    """
    A = A + np.diag(np.ones(A.shape[0], dtype=np.float32))  # 给邻接矩阵加上单位矩阵（自环）
    D = np.array(np.sum(A, axis=1)).reshape((-1,))  # 计算节点的度矩阵 D，即每个节点的出度之和（加了自环后）
    D[D <= 10e-5] = 10e-5    # Prevent infs 防止除以 0
...
def generate_dataset(X, num_timesteps_input, num_timesteps_output, means, stds, inter_step):
    ...
    # 计算样本窗口起止位置，最终生成若干个 (start_idx, end_idx) 对，表示每个样本的时间范围
    ...
    features, target = [], []  # 分别用于存放所有样本的 x（输入）和 y（目标）
    for i, j in indices:
        features.append(  # 提取输入序列，形状为 [N, num_timesteps_input, 2]
            # [N, 2, L] -> [N, L, 2]
            X[:, :, i: i + num_timesteps_input].transpose(
                (0, 2, 1)))
        # target only add true value 提取输出目标值（反归一化后），仅取 speed 特征（即 channel 0）
        target.append(X[:, 0, i + num_timesteps_input: j]*stds[0]+means[0])

    x = torch.from_numpy(np.array(features)).float()  # x shape: [B, N, input_len, 2]，B 是样本数量（由窗口滑动得到）
    y = torch.from_numpy(np.array(target)).float()  # y shape: [B, N, output_len]
