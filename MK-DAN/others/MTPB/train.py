...
parser.add_argument('--finetune_epochs', default=100,type=int)  # 300
...
parser.add_argument('--seed',default=12,type=int)  # 7
parser.add_argument('--data_list', default='nycbike_washington_chicago',type=str)
...
if __name__ == '__main__':
    ...
    log_config_dir = os.path.join(args.save_dir, "configs")
    os.makedirs(log_config_dir, exist_ok=True)
    yaml.dump([dict(vars(args)), config], open(os.path.join(log_config_dir, "config.yaml"), "w"))

    # yaml.dump([dict(vars(args)), config], open(os.path.join(args.save_dir, "configs/config.yaml"), "w"))
