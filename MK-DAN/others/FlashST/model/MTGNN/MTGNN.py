...
class MTGNN(nn.Module):
    def __init__(self, args, dim_in, dim_out, A_dict, dataset_use, dataset_test, mode):
        ...
        if mode == 'pretrain':
            ...
            for i, data_graph in enumerate(dataset_use):
                self.dataset2index2[data_graph] = i
                n_dataset = A_dict[data_graph].shape[0]
                self.idx_pretrain.append(torch.arange(n_dataset).to(self.device))
        else:
            self.idx_eval = []
            for i, data_graph in enumerate([dataset_test]):
                self.dataset2index2[data_graph] = i
                n_dataset = A_dict[data_graph].shape[0]
                self.idx_eval.append(torch.arange(n_dataset).to(self.device))
        ...
        self.norm_pretrain = nn.ModuleList()
        self.norm_eval = nn.ModuleList()
        ...
        self.dataset2index = {}
        if mode == 'pretrain':
            self.gc_pretrain = nn.ModuleList()
            for i, data_graph in enumerate(dataset_use):
                self.dataset2index[data_graph] = i
                n_dataset = A_dict[data_graph].shape[0]
                # print(n_dataset, data_graph)
                self.gc_pretrain.append(GraphConstructor(n_dataset, self.subgraph_size, self.node_dim,
                                       self.device, alpha=self.tanhalpha, static_feat=self.static_feat))
        else:
            self.gc_eval = nn.ModuleList()
            for i, data_graph in enumerate([dataset_test]):
                self.dataset2index[data_graph] = i
                n_dataset = A_dict[data_graph].shape[0]
                self.gc_eval.append(GraphConstructor(n_dataset, self.subgraph_size, self.node_dim,
                                                         self.device, alpha=self.tanhalpha,
                                                         static_feat=self.static_feat))
        # if self.mode == 'eval':
        #     self.gc = GraphConstructor(self.num_nodes, self.subgraph_size, self.node_dim,
        #                                self.device, alpha=self.tanhalpha, static_feat=self.static_feat)
        # else:
        #     self.gc1 = GraphConstructor(self.num_nodes, self.subgraph_size, self.node_dim,
        #                                self.device, alpha=self.tanhalpha, static_feat=self.static_feat)
        ...
        for i in range(1):
            ...
            for j in range(1, self.layers+1):
                ...
                if mode == 'pretrain':
                    if self.input_window > self.receptive_field:
                        for j, data_graph in enumerate(dataset_use):
                            self.dataset2index1[data_graph] = j
                            n_dataset = A_dict[data_graph].shape[0]
                            self.norm_pretrain.append(self.norm.append(LayerNorm((self.residual_channels, n_dataset,
                        ...
                        for j, data_graph in enumerate(dataset_use):
                            self.dataset2index1[data_graph] = j
                            n_dataset = A_dict[data_graph].shape[0]
                            self.norm_pretrain.append(LayerNorm((self.residual_channels, n_dataset,
                                                    self.receptive_field - rf_size_j + 1),
                                                   elementwise_affine=self.layer_norm_affline))
                else:
                    if self.input_window > self.receptive_field:
                        for i, data_graph in enumerate([dataset_test]):
                            self.dataset2index1[data_graph] = i
                            n_dataset = A_dict[data_graph].shape[0]
                            self.norm_eval.append(LayerNorm((self.residual_channels, n_dataset,
                                                    self.input_window - rf_size_j + 1),
                                                   elementwise_affine=self.layer_norm_affline))
                    else:
                        for i, data_graph in enumerate([dataset_test]):
                ...
                # else:
                #     if self.input_window > self.receptive_field:
                #         self.norm1.append(LayerNorm((self.residual_channels, self.num_nodes,
                #                                     self.input_window - rf_size_j + 1),
                #                                    elementwise_affine=self.layer_norm_affline))
                #     else:
                #         self.norm1.append(LayerNorm((self.residual_channels, self.num_nodes,
    ...
    def forward(self, source, select_dataset, idx=None):
        ...
        if self.gcn_true:
            if self.mode == 'pretrain':
                if self.buildA_true:
                    if idx is None:
                        adp = self.gc_pretrain[self.dataset2index[select_dataset]](self.idx_pretrain[self.dataset2index2[select_dataset]])
                        # print(adp.shape)
                    else:
                        adp = self.gc_pretrain[self.dataset2index[select_dataset]](idx)
                else:
                    adp = self.predefined_A
            else:
                if self.buildA_true:
                    if idx is None:
                        adp = self.gc_eval[self.dataset2index[select_dataset]](self.idx_eval[self.dataset2index2[select_dataset]])
                    else:
                        adp = self.gc_eval[self.dataset2index[select_dataset]](idx)
                else:
                    adp = self.predefined_A
        # print(inputs.shape)
        ...
        for i in range(self.layers):
            ...
            skip = s + skip
            if self.gcn_true:
                # print(adp.shape, select_dataset)
            ...
            if self.mode == 'pretrain':
                ...
                if idx is None:
                    x = self.norm_pretrain[self.dataset2index1[select_dataset] + (i*len(self.dataset2index1))](x, self.idx_pretrain[self.dataset2index2[select_dataset]])
                else:
                    x = self.norm_pretrain[self.dataset2index1[select_dataset] + (i*len(self.dataset2index1))](x, idx)
            else:
                if idx is None:
                    x = self.norm_eval[self.dataset2index1[select_dataset] + (i*len(self.dataset2index1))](x, self.idx_eval[self.dataset2index2[select_dataset]])
                else:
                    x = self.norm_eval[self.dataset2index1[select_dataset] + (i*len(self.dataset2index1))](x, idx)
