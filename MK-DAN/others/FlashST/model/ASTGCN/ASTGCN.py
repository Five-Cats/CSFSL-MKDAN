...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.Ve.shape, product.shape, self.be.shape)
...
class ASTGCN(nn.Module):
    ...
    def __init__(self, args, A, num_of_vertices, len_input, num_for_predict, dim_in, dim_out, DEVICE):
        '''
        :param nb_block:
        :param in_channels:
        :param K:
        ...
        self.BlockList = nn.ModuleList([ASTGCN_block(DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, self.cheb_polynomials, num_of_vertices, len_input)])

        self.BlockList.extend([ASTGCN_block(DEVICE, nb_time_filter, K, nb_chev_filter, nb_time_filter, 1, self.cheb_polynomials, num_of_vertices, len_input//time_strides) for _ in range(nb_block-1)])

        self.final_conv = nn.Conv2d(int(len_input/time_strides), num_for_predict*dim_out, kernel_size=(1, nb_time_filter))
        # self.final_conv = nn.Conv2d(nb_time_filter, dim_out, kernel_size=(1, 1))
        self.DEVICE = DEVICE
    ...
    def forward(self, x, select_dataset):
        ...
        # output = self.final_conv(x.permute(0, 2, 1, 3)).transpose(1, 3)
        return output
