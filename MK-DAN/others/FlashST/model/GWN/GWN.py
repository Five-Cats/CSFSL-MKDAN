...
class GWNET(nn.Module):
    def __init__(self, args, dim_in, dim_out, A_dict, dataset_use, dataset_test, mode):
        ...
        self.start_conv = nn.Conv2d(in_channels=self.feature_dim,
                                    ...
                                    kernel_size=(1, 1))
        # self.U2 = nn.Parameter(torch.randn(self.residual_channels, self.feature_dim).to(self.device))
        # self.Lin = nn.Linear(self.feature_dim, 1)
            ...
            if self.aptinit is None:
                ...
                if mode == 'pretrain':
                    ...
                    for i, data_graph in enumerate(dataset_use):
                        ...
                        self.nodevec1_pretrain.append(nn.Parameter(torch.randn(n_dataset, 10).to(self.device),
                                             requires_grad=True))
                        self.nodevec2_pretrain.append(nn.Parameter(torch.randn(10, n_dataset).to(self.device),
                                             requires_grad=True))
                else:
                    ...
                    for i, data_graph in enumerate([dataset_test]):
                        self.dataset2index[data_graph] = i
                        n_dataset = A_dict[data_graph].shape[0]
                        self.nodevec1_eval.append(nn.Parameter(torch.randn(n_dataset, 10).to(self.device),
                                             requires_grad=True))
                        self.nodevec2_eval.append(nn.Parameter(torch.randn(10, n_dataset).to(self.device),
                                             requires_grad=True))
    ...
    def forward(self, source, select_dataset):
        ...
        if in_len < self.receptive_field:
            ...
            x = inputs
        # x = self.Lin(x.transpose(1, 3)).transpose(1, 3)
        # print(x.shape)
        ...
        if self.gcn_bool and self.addaptadj and self.supports is not None:
            ...
            if self.mode == 'pretrain':
                adp = F.softmax(F.relu(torch.mm(self.nodevec1_pretrain[self.dataset2index[select_dataset]],
                                                self.nodevec2_pretrain[self.dataset2index[select_dataset]])), dim=1)
            else:
                adp = F.softmax(F.relu(torch.mm(self.nodevec1_eval[self.dataset2index[select_dataset]],
