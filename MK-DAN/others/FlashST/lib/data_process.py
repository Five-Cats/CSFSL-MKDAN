...
def data_type_init(DATASET, args):
    if DATASET == 'METR_LA' or DATASET == 'SZ_TAXI' or DATASET == 'PEMS07M':
        data_type = 'speed'
    elif DATASET == 'PEMS08' or DATASET == 'PEMS04' or DATASET == 'PEMS03' or DATASET == 'PEMS07' or DATASET == 'nyc_bike0' or DATASET == 'DCwashington' or DATASET == 'Chicago' or DATASET == 'LABike':
        data_type = 'flow'
...
def load_st_dataset(dataset, args):
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
    elif dataset == 'nyc_bike0':
        data_path = os.path.join('../data/nyc_bike0/nyc_bike0.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
    elif dataset == 'nyc_bike1':
        data_path = os.path.join('../data/nyc_bike1/nyc_bike1.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
    elif dataset == 'nyc_bike2':
        data_path = os.path.join('../data/nyc_bike2/nyc_bike2.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
        ...
        # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
    elif dataset == 'DCwashington':
        data_path = os.path.join('../data/DCwashington/DCwashington.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
        ...
        data_path = os.path.join('../data/Chicago/Chicago.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
        ...
        data_path = os.path.join('../data/LABike/LABike.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
    ...
    # 4 / 1 / 2023 - 4 / 30 / 2023 Saturday
        ...
        data_path = os.path.join('../data/NYCBike/NYCBike.npz')
        data = np.load(data_path)['data'][..., 0].astype(np.float64)
        week_start = 6
        weekday_only = False
        interval = 20
...
def get_pretrain_task_batch(args, x_list, y_list):
    ...
    for index in range(0, len_dataset, batch_size):
        ...
        # print(type(indices))
        # print(type(x_list[select_dataset]))
        # x_data = x_list[select_dataset][indices.copy()]
        # y_data = y_list[select_dataset][indices.copy()]
        x_data = x_list[select_dataset][torch.tensor(indices.copy(), dtype=torch.long)]
        y_data = y_list[select_dataset][torch.tensor(indices.copy(), dtype=torch.long)]
