...
np.savez('LABike.npz', data=bike_demand)

datas = np.load('LABike.npz')
print(datas.files)
print(datas['data'])
print(datas['data'].shape)

np.savetxt("LABike.csv", bike_graph, delimiter=",")
A = pd.read_csv("LABike.csv", header=None).values.astype(np.float32)
print(A.shape)
print(A)
