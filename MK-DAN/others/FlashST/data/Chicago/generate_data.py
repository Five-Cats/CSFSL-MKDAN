...
np.savez('Chicago.npz', data=bike_demand)

datas = np.load('Chicago.npz')
print(datas.files)
print(datas['data'])
print(datas['data'].shape)

np.savetxt("Chicago.csv", bike_graph, delimiter=",")
A = pd.read_csv("Chicago.csv", header=None).values.astype(np.float32)
print(A.shape)
print(A)
