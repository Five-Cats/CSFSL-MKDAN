...
np.savez('nyc_bike2.npz', data=bike_demand)

datas = np.load('nyc_bike2.npz')
print(datas.files)
print(datas['data'])
print(datas['data'].shape)

np.savetxt("nyc_bike2.csv", bike_graph, delimiter=",")
A = pd.read_csv("nyc_bike2.csv", header=None).values.astype(np.float32)
print(A.shape)
print(A)
