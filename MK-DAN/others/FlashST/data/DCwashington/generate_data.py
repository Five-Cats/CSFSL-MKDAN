...
np.savez('DCwashington.npz', data=bike_demand)

datas = np.load('DCwashington.npz')
print(datas.files)
print(datas['data'])
print(datas['data'].shape)

np.savetxt("DCwashington.csv", bike_graph, delimiter=",")
A = pd.read_csv("DCwashington.csv", header=None).values.astype(np.float32)
print(A.shape)
print(A)
