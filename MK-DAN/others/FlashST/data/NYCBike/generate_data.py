...
np.savez('NYCBike.npz', data=bike_demand)

datas = np.load('NYCBike.npz')
print(datas.files)
print(datas['data'])
print(datas['data'].shape)

np.savetxt("NYCBike.csv", bike_graph, delimiter=",")
A = pd.read_csv("NYCBike.csv", header=None).values.astype(np.float32)
print(A.shape)
print(A)
